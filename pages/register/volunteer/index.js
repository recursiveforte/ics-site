...
import { manifest, requiredList } from '../../../lib/manifest_volunteers'
    ...
    return (
        <Layout>
            ...
            <Card px={[2, 4]} pb={[4, 4]} pt={[4,2]} sx={{
                ...
                <Box my={3}><Text variant={"title"} color={"primary"}>Volunteer Registration</Text></Box>
                <Box bg="sunken" p={3} mb={3} sx={{ borderRadius: 3 }}>
                    <Text sx={{fontSize: "18px"}}>
                        Thank you for your interest in volunteering at Ice Cream Social! We're CHARGED UP to run this event, and excited to have you volunteering!
                    ...
                    onClick={() => {
                        ...
                        fetch('/api/submit_volunteers', {
                            ...
                            .then(
                                ({ success, error }) => {
                                    setDisabled(false);
                                    success ? window.location.replace('/register/volunteer/success') : toast.notify(error, { type: 'error', title: 'Oops!', duration: 60 })
                                }
                            )
                    }}
                    bg="green"
